"""
Search for counterexamples: switches satisfying C1 + C2 + row_symmetry that are NOT cospectral.
Restricted to min degree 2 graphs.
"""
...
def find_switches_with_conditions(G, tol=1e-6):
    """Find switches satisfying C1 + C2 + row_symmetry."""
    results = []
...
for n in range(4, 9):
    ...
    for g6 in graphs:
        ...
        for v1, w1, v2, w2, Gp in find_switches_with_conditions(G):
            n_sw += 1
...
if counterexamples:
    ...
    for g6, sw, k in counterexamples[:10]:
        print(f"  {g6}, switch {sw}, diverges at k={k}")
