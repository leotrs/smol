"""
Test: C1 + C2 + ext_degs match + non-empty v1w1 and w1w2
      + NOT all intersection degs identical (heterogeneity)
"""
    ...
    for e1, e2 in combinations(G.edges(), 2):
        for v1, w1 in [e1, e1[::-1]]:
            for v2, w2 in [e2, e2[::-1]]:
                ...
                # Heterogeneity: not all four are identical
                if int_degs_v1w1 == int_degs_v2w2 == int_degs_v1w2 == int_degs_v2w1:
                    # If all identical and size > 1, might be problematic
                    if len(v1w1) > 1 and len(set(int_degs_v1w1)) == 1:
                        # All same degree in intersection - skip
                        continue
