"""
Test: C1 + C2 + v1w1 non-empty + w1w2 non-empty 
      + diagonal intersection degree sequences match
      + NOT(both ext(v) ⊆ w1∩w2)
"""
    ...
    for e1, e2 in combinations(G.edges(), 2):
        for v1, w1 in [e1, e1[::-1]]:
            for v2, w2 in [e2, e2[::-1]]:
                ...
                # Diagonal intersection degree sequences must match
                ...
                if int_degs_v1w1 != int_degs_v2w2:
                    continue
                if int_degs_v1w2 != int_degs_v2w1:
                    continue
                
                # NOT(both ext(v) subset of w1∩w2)
                w1w2 = ext[w1] & ext[w2]
