"""
Test (C3-row): Row symmetry condition.
- ws(ext(v1) ∩ ext(w1)) = ws(ext(v1) ∩ ext(w2))
- ws(ext(v2) ∩ ext(w1)) = ws(ext(v2) ∩ ext(w2))
Plus triangle equality.
"""
...
def find_c3row_switches(G, tol=1e-9):
    results = []
...
for n in range(4, 9):
    ...
    for g6 in graphs:
        ...
        for v1, w1, v2, w2, Gp in find_c3row_switches(G):
            n_sw += 1
...
print(f"\nTOTAL: {total} (C1+C2+C3-row+tri) switches")
