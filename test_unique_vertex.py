"""
Test: C1 + C2 + ext_degs match + non-empty intersections 
      + NOT(ext(v) ⊆ w1∩w2) 
      + at least one intersection vertex appears only once
"""
    ...
    for e1, e2 in combinations(G.edges(), 2):
        for v1, w1 in [e1, e1[::-1]]:
            for v2, w2 in [e2, e2[::-1]]:
                ...
                # Neither v's ext can be subset of w1∩w2
                w1w2 = ext[w1] & ext[w2]
                if ext[v1] <= w1w2 and ext[v2] <= w1w2:
                    continue
                
                # NEW: At least one intersection vertex appears only once
                inters = [ext[v1] & ext[w1], ext[v2] & ext[w2], 
                          ext[v1] & ext[w2], ext[v2] & ext[w1]]
                vertex_counts = Counter()
                for inter in inters:
                    for v in inter:
                        vertex_counts[v] += 1
                if vertex_counts and min(vertex_counts.values()) >= 2:
                    continue
